...
export default function SpendingPieChart({ selectedWallet }: { selectedWallet?: string | null }) {
  const now = new Date();
  const [month, setMonth] = useState(now.getMonth());
  const [year] = useState(now.getFullYear());

  const { data = [], isLoading } = useSpendingByCategory(year, month + 1, selectedWallet);
